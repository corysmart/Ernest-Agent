...
export async function buildContainer(options: BuildContainerOptions = {}): Promise<ContainerContext> {
  ...
  if (process.env.DATABASE_URL && memoryRepository instanceof PostgresMemoryRepository) {
    ...
    try {
      ...
      while (hasMore) {
        ...
        // P2: Ensure concurrency is at least 1 to prevent infinite loop (fractional values like 0.5 floor to 0)
        ...
        // P2: Floor and clamp to at least 1 to prevent infinite loop in for (i += maxConcurrency)
        const floored = Number.isFinite(concurrencyLimitRaw) && concurrencyLimitRaw > 0 
          ? Math.floor(concurrencyLimitRaw) 
          : 10;
        const maxConcurrency = floored >= 1 ? floored : 1; // Ensure at least 1 to prevent infinite loop
  ...
  const permissionGate = new ToolPermissionGate({
    allow: [
      'pursue_goal', 'invoke_codex', 'send_email', 'schedule_task', 'get_recent_runs',
      'create_test_email_account', 'save_email_config', 'read_file', 'list_dir', 'run_command', 'write_file',
      'create_workspace', 'sync_heartbeat_archive',
      'complete_run'
    ]
  });
