...
describe('LocalLLM Allowlist Runtime Validation', () => {
  ...
  it('caches DNS validation per allowlist configuration', async () => {
    ...
    // Create adapter with allowlist
    const adapter1 = await LocalLLMAdapter.create({
      baseUrl: 'https://localhost:11434',
      allowlist: ['localhost']
    });

    // Create adapter without allowlist (different cache key)
    const adapter2 = await LocalLLMAdapter.create({
      baseUrl: 'https://api.example.com',
      resolveDns: false
    });

    // First adapter should not call DNS (allowlist bypasses)
    await adapter1.generate({ messages: [{ role: 'user', content: 'hi' }] });
    expect(isSafeUrlSpy).not.toHaveBeenCalled();

    // Second adapter should call DNS (no allowlist)
