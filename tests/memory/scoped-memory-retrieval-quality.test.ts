...
describe('Scoped Memory Retrieval Quality', () => {
  ...
  it('returns fewer results when scope has fewer memories than requested', async () => {
    const repository = new InMemoryMemoryRepository();
    const vectorStore = new LocalVectorStore();
    const embeddingProvider = new MockLLMAdapter();
    ...
    // Should get only 2 results (all available)
  ...
  it('handles case where other scopes completely dominate top-K results', async () => {
    ...
    // The multiplier ensures we query enough results to find scope A's memory
